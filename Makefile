...
init:
	...
	ln -sf $(BASE_DIR)/scripts/$(RUN_MODE)/slurm_scRNA_pipeline.sh runs/$(RUN_NAME)/slurm_scRNA_pipeline.sh
	ln -sf $(BASE_DIR)/scripts/$(RUN_MODE)/slurm_scRNA_get_fastqs.sh runs/$(RUN_NAME)/slurm_scRNA_get_fastqs.sh
	ln -sf $(BASE_DIR)/scripts/$(RUN_MODE)/slurm_CellBenderGPU.sh runs/$(RUN_NAME)/slurm_CellBenderGPU.sh
	cat scripts/$(RUN_MODE)/scRNA_params.sh \
...
init_report:
	...
	$(BASE_DIR)/scripts/$(RUN_MODE)/scRNA_create_run_csv.R $(BASE_DIR) $(RUN_NAME)
...
qcs_to_ppt:
	$(BASE_DIR)/scripts/$(RUN_MODE)/scRNA_qcs_to_ppt.R $(BASE_DIR)/runs/$(RUN_NAME) $(HG_MM_MODE)
...
report: qcs_to_ppt
	$(BASE_DIR)/scripts/$(RUN_MODE)/scRNA_make_report.R $(BASE_DIR) $(RUN_NAME)
