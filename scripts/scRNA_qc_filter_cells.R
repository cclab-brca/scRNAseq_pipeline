...
if (hg_pref != "NA" && mm_pref != "NA" && sum(is_hg) > 20 && sum(!is_hg) > 20) {
  ...
  plot(log2(md$hg_tot_umis[is_hg]), md$hg_f_mito[is_hg], pch=19, cex=0.5, col=ifelse(md$hg_f_mito[is_hg] >= max_hg_f_mito, 'red', 'black'), xlab="UMIs (log2)", ylab="%mito", main=sprintf("%s: Human (%d)", sample_name, sum(is_hg)), ylim=0:1)
  ...
  par(mai=c(pm[1], pm[2]+pm[4]-0.1, pm[3], 0.1))
  plot(log2(md$mm_tot_umis[!is_hg]), md$mm_f_mito[!is_hg], pch=19, cex=0.5, col=ifelse(md$mm_f_mito[!is_hg] >= max_mm_f_mito, 'red', 'black'), xlab="UMIs (log2)", ylab="%mito", main=sprintf("%s: Mouse (%d)", sample_name, sum(!is_hg)), ylim=0:1)
